# Urban Explorer Frontend

[![CI](https://github.com/ismaeldosil/urban-explorer-frontend/actions/workflows/ci.yml/badge.svg)](https://github.com/ismaeldosil/urban-explorer-frontend/actions/workflows/ci.yml)
[![Release](https://github.com/ismaeldosil/urban-explorer-frontend/actions/workflows/release.yml/badge.svg)](https://github.com/ismaeldosil/urban-explorer-frontend/actions/workflows/release.yml)
[![Ionic](https://img.shields.io/badge/Ionic-8.0-3880FF?logo=ionic&logoColor=white)](https://ionicframework.com/)
[![Angular](https://img.shields.io/badge/Angular-20-DD0031?logo=angular&logoColor=white)](https://angular.io/)
[![Capacitor](https://img.shields.io/badge/Capacitor-7.0-119EFF?logo=capacitor&logoColor=white)](https://capacitorjs.com/)

> Mobile app for Urban Explorer - Gamified urban exploration

## Quick Start

```bash
npm install
npm start
# Opens http://localhost:8100
```

## Documentation

All documentation is centralized in **[urban-explorer-docs](../urban-explorer-docs/)**:

| Document | Description |
|----------|-------------|
| [Full README](../urban-explorer-docs/frontend/FULL-README.md) | Complete project documentation |
| [Architecture](../urban-explorer-docs/frontend/architecture/ESTRUCTURA-CLEAN-ARCHITECTURE.md) | Clean Architecture guide |
| [CI/CD Setup](../urban-explorer-docs/frontend/ci-cd/CI-CD-SETUP.md) | CI/CD configuration |
| [Deep Linking](../urban-explorer-docs/frontend/deep-linking/) | Deep linking guides |
| [Screens Design](../urban-explorer-docs/frontend/screens/) | UI/UX screen designs |

## Tech Stack

- **Ionic 8** - Mobile UI framework
- **Angular 20** - Web framework
- **Capacitor 7** - Native runtime (iOS/Android)
- **Supabase** - Backend (Auth, DB, Storage)

## Available Scripts

| Command | Description |
|---------|-------------|
| `npm start` | Development server |
| `npm run build` | Production build |
| `npm run lint` | Run ESLint |
| `npm test` | Unit tests |

## License

MIT License - See [LICENSE](LICENSE) for details.
