...
export class SupabaseLocationRepository implements ILocationRepository {
  ...
  async search(
    ...
    // Normalize query - remove accents for better matching
    const normalizedQuery = query.toLowerCase().trim();

    // Build OR conditions for each word in the query
    const words = normalizedQuery.split(/\s+/).filter(w => w.length >= 2);
    const searchTerm = words.length > 0 ? words[0] : normalizedQuery;
    ...
    let dbQuery = this.supabase
      .from('locations')
      .select('*')
      .or(`name.ilike.%${searchTerm}%,description.ilike.%${searchTerm}%,address.ilike.%${searchTerm}%,category.ilike.%${searchTerm}%`);
